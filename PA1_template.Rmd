...
After a quick computation, it looks like there are about `r nrow(nonvalues)` rows missing important details.  Let's set up a second dataset, similar to the first, albeit where our nonvalues have been substituted with the mean steps for that same interval.
...
Taking a look, now, at our theorical new results, our new mean daily steps of `r imputed_mean_steps` and median daily steps of `r imputed_median_steps` being so close to the original untreated data lend some credence to veracity of these estimated inclusions.  The data seems safe to proceeding with.
